...
export class ClientTypeService {
   ...
   url = environment.api + 'clienttype/';
   ...
   post(clienttype: ClientType): Promise<any> {
      return this.http.post(this.url, JSON.stringify(clienttype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(clienttype: ClientType): Promise<any> {
      return this.http.put(this.url, JSON.stringify(clienttype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
