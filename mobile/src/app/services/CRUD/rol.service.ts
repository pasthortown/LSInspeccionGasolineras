...
export class RolService {
   ...
   post(rol: Rol): Promise<any> {
      return this.http.post(this.url, JSON.stringify(rol), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(rol: Rol): Promise<any> {
      return this.http.put(this.url, JSON.stringify(rol), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
