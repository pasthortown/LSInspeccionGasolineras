...
export class ClientService {
   ...
   post(client: Client): Promise<any> {
      return this.http.post(this.url, JSON.stringify(client), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
   ...
   put(client: Client): Promise<any> {
      return this.http.put(this.url, JSON.stringify(client), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
