...
export class GenderService {
   ...
   post(gender: Gender): Promise<any> {
      return this.http.post(this.url, JSON.stringify(gender), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(gender: Gender): Promise<any> {
      return this.http.put(this.url, JSON.stringify(gender), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
